...
namespace AbstractShopService.ImplementationsBD
{
    public class ReportServiceBD : IReportService
    {
        ...
        public void SaveSectionPrice(ReportBindingModel model)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < Sections.Count; ++i)
                {
                    ...
                    table.Cell(i + 1, 2).Range.Text = Sections[i].Price.ToString();
                }
        ...
        public List<BonusFineBlocksLoadViewModel> GetBonusFineBlocksLoad()
        {
            return context.BonusFineBlocks
                            .ToList()
                            .GroupJoin(
                                    context.BonusFineBlockPayments
                                                .Include(r => r.Zakazchik)
                                                .ToList(),
                                    BonusFineBlock => BonusFineBlock,
                                    BonusFineBlockPayment => BonusFineBlockPayment.BonusFineBlock,
                                    (BonusFineBlock, BonusFineBlockCompList) =>
            new BonusFineBlocksLoadViewModel
            {
                BonusFineBlockName = BonusFineBlock.BonusFineBlockName,
                TotalCount = BonusFineBlockCompList.Sum(r => r.Count),
                Components = BonusFineBlockCompList.Select(r => new BonusFineBlocksPaymentLoadViewModel { PaymentName = r.Zakazchik.ZakazchikFIO, Count = r.Count }).ToList()
            })
                            .ToList();
        }
        ...
        public void SaveBonusFineBlocksLoad(ReportBindingModel model)
        {
            ...
            try
            {
                ...
                excelcells.Value2 = "Назначенные бонусы и штрафы. Блокировка.";
                ...
                var dict = GetBonusFineBlocksLoad();
                if (dict != null)
                {
                    ...
                    foreach (var elem in dict)
                    {
                        ...
                        excelcells.Value2 = elem.BonusFineBlockName;
                        ...
                        if (elem.Components.Count() > 0)
                        {
                            ...
                            {
                                excelcells.Value2 = listElem.PaymentName;
        ...
        public void SaveZakazchikZakazs(ReportBindingModel model)
        {
            ...
            //вставляем заголовок
            var phraseTitle = new Phrase("Заявки клиентов",
            ...
            table.AddCell(new PdfPCell(new Phrase("Кружок", fontForCellBold))
            {
                HorizontalAlignment = Element.ALIGN_CENTER
            });
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].SectionName, fontForCells));
                table.AddCell(cell);
